...
- [SP 101(all new version)](#sp-101all-new-version)
  ...
  - [Programming](#programming)
    ...
    - [Shell \& Linux](#shell--linux)
      - [I/O](#io)
  - [CDISC Standerd](#cdisc-standerd)
    - [SDTM Standerd](#sdtm-standerd)
      - [临床试验流程](#临床试验流程)
      - [一些基本原理](#一些基本原理)
      - [Role](#role)
      - [Origin](#origin)
      - [Core](#core)
      - [3 General Obs Class](#3-general-obs-class)
      - [4 Special Purpose](#4-special-purpose)
      - [WHODrug](#whodrug)
      - [MedDRA](#meddra)
      - [DM](#dm)
      - [SV](#sv)
  ...
  - [SP Interview Exp](#sp-interview-exp)
...
> 文档最佳实践指南
>
>- 使用vscode中markdownlint进行markdown语法检查。
>- 尽可能少使用段落标题以方面快速索引，因为基础目录可能会很长。
>- 尽量减少插入图片，方便迁移。
>- 只使用markdown基础语法和简单扩展语法。
>- 代码块通常采用2个空格或一个制表符缩进。
>- 块引用可以包含其他Markdown格式的元素。
>- 强调：斜体（1个星号）；加粗（2个星号）；删除线（两个波浪号）。
>- 段落：不要用空格或制表符缩进段落。
>- 表格：使用多个连字符创建标题，并使用竖线分隔每列，连字符的左/侧/两侧添加冒号进行对齐。
>- 脚注：在方括号（[^1]）内添加插入符号和标识符。
>- 定义列表：在第一行上键入术语。在下一行键入一个冒号。
>- 任务列表：- [ ] 。在方括号之间添加x表示选择一个复选框。
...
- Required：必须有，非空值
是标识数据记录的变量（essential key variable and topic variable)或者是使记录有意义所必需的变量。必须始终包含在数据集中，并且对任何记录都不能为空。

- Expected：必须有，可为空值
是使记录在特定domain里面的上下文中有用的变量。可能包含一些空值，但在大多数情况下不应该包含空值。当研究不包括expected variables的data item时，仍然要在数据集中包含一个空列，并且填写comment到define.xml去说明study不包括该data item。

- Permissible：若无特殊规定可有
在适当的SDTM数据集中使用。尽管域规范表只列出了SDTM中列出的部分标识符、时间和一般观察类变量，但除非在本实现指南中有特别的限制或特定的域假设。
...
- 起源与发展
  ...
  - 结构层次
    ...
    ```Mermaid
    flowchart TD
    A[SOC / System Organ Class / 系统器官分类]-->B[HLGT / High Level Group Term / 高位组语]
    B-->C[HLT / High Level Term / 高位语]
    C-->D[PT / Preferred Term / 首选语]
    D-->E[LLT / Lowest Level Term / 低位语]
    A---A1([--BODSYS / --BDSYCD])
    B---B1([--HLGT / --HLGTCD])
    C---C1([--HLT / --HLTCD])
    D---D1([--DECOD / --PTCD])
...
- 术语特点
  ...
  1. 细分化：最低级别术语LLT提供了非常具体、细致的医学术语，尽可能地接近报告的字面语言。
  2. 多轴性：即一个术语出现在多个SOC中，并根据不同的类别分组（如：按病因或发病部位）。

    ```Mermaid
    flowchart TB
          A[SOC 呼吸系统/胸及纵隔疾病]--HLGT/HLT--->C[PT 流行性感冒]
          B[SOC 感染及侵染类疾病]--HLGT/HLT--->C
          C --> LLT
    ```

  3. 所有PT都有唯一的主SOC（primary SOC）：一个PT尽管可属于多个SOC分类，但其中只有一个被指定为主SOC（primary SOC），余为次SOC（secondary SOC）。

  特别指出，27个SOC中，3个没有多轴性，即“各类检查”，“各种手术与医疗操作”，“社会环境”。
...
- 数据集中的MedDRA
  ...
  AETERM是搜集时的术语。
  经过MedDRA编码之后，变成标准化的术语AEDECOD, 又称为Preferred Term(PT)。
  而该PT所属的系统器官分类SOC(System Organ Class)在数据集中通过变量AEBODSYS体现。
...
- 标准变量
  ...
  >- **AETERM(topic)** AEMODIFY AELLT AELLTCD AEDECOD AEPTCD AEHLT AEHLTCD AEHLGT AEHLGTCD
...
- AE的定义：
  - 受试者**接受试验用药品后**出现的所有不良医学事件（强调时间关联性，不一定有因果关系）。
    >- 临床体征和症状：疼痛、发热、皮疹
    >- 体格检查发现：超声异常、骨折
...
- AE收集
  ...
  - 必须至少提供以下信息：
    ...
    - Severity (AESEV)
    - Action taken (AEACN)
    - Relation to the drug (AEREL)
    - Resolution (AEOUT)
  - AE Coding
    - AETERM(required, verbatim term)
    - AEMODIFY(permissible, modification of a verbatim term for coding)
    - AEDECOD(required, preferred term)
    - AEBODSYS(expected, system organ class)
    -
...
- SAE
  ...
  - 必须建立专门的安全数据库 (药物警戒PV) 来记录与SAE有关的信息。
